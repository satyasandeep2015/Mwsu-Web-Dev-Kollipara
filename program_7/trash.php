				...
				<div class="panel-body" > 
					...
					<div class="row">
						<div class="text-center">
							...
							<div class="col-xs-4">
								<button type="button" class="updatebtn btn-default btn-sm btn-block">
									Update cart
								</button>
							</div>
						</div>
...
var item = [];
    ...
    $('.updatebtn').click(function(){
		...
		postData['uid']=guid;
		postData['pid']=item[0];
		postData['count']=itemcount;
		postData['description']=item[1];
		postData['price']=item[3];
		postData['time-added']=Math.floor(Date.now() / 1000);
		
		console.log(postData);
		cartTotal = parseFloat($('#cart-total').text());
		if(isNaN(cartTotal))
			cartTotal = 0;

		cartTotal += (parseFloat(item[3])*itemcount);
	...
	function addCartItem(item){
		...
		'<div class="row" id="item-'+item[0]+'">'+
			...
			'		<h6><strong>$'+item[3]+'<span class="text-muted">x</span></strong></h6>'+
			'	</div>'+
			'	<div class="col-xs-5">'+
			'		<input type="text" class="form-control input-sm"  id="count" value="1">'+
			'	</div>'+
			'	<div class="col-xs-2">'+

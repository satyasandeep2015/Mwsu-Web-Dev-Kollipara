...
(function($) {
    ...
    */
    ...
    function loadTableData(page, page_size,sort,order) {
		...
		console.log("http://mywebdevcourse.xyz/api/api.php/products?order="+sort+"&page=" + page + "," + page_size);
        $.get("http://mywebdevcourse.xyz/api/api.php/products?order="+sort+order+"&page=" + page + "," + page_size)
        ...
        .done(function(data) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                col_head += "<th class=\"sort-up\" >" + cols[i];
            ...
            for (var i = 0; i < records.length; i++) {
                ...
                rows = rows + '<td style="vertical-align:middle" nowrap><i class="fa fa-pencil-square-o" aria-hidden="true"></i> <i class=\"fa fa-trash-o del\" aria-hidden="true"></i></td>';
			...
			$('.del').click(function(){
                ...
                var b = $(this).closest('tr').children().text();
				console.log(a);
                //console.log(b);
				//var column = $this.text();
                ...
                var result = confirm("Want to delete? \n "+b);
                    if (result) {                    
                        $.ajax({
                        url: 'http://mywebdevcourse.xyz/api/api.php/products/'+a,
						//console.log('http://mywebdevcourse.xyz/api/api.php/products/'+2);
                        type: 'DELETE',
                        //data: {movie:movie}, //<-----this should have to be an objec
                        contentType:'application/json',  // <---add this
                        dataType: 'json',                // <---update this
                        success: window.alert("successfully deleted")
                    });
                    }
                    else
                    {
    ...
    function getTotalPages(){
        $.get("http://mywebdevcourse.xyz/api/api.php/products")
